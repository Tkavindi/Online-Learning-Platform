...
const CoursePage = () => {
  ...
  const [editingCourse, setEditingCourse] = useState(null);
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [enrollingCourseIds, setEnrollingCourseIds] = useState(new Set());
  ...
  const handleEnroll = async (courseId, courseTitle, e) => {
    e.stopPropagation();
    if (enrollingCourseIds.has(courseId)) return; // Prevent double clicks
    setEnrollingCourseIds((prev) => new Set(prev).add(courseId));
    try {
      await axios.post(
        `${BASE_URL}/api/enrollments/${courseId}`,
        {},
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="ml-6 flex-shrink-0">
                  {user?.role === "student" && (
                    <button
                      onClick={(e) => handleEnroll(course._id, course.title, e)}
                      disabled={course.isEnrolled || enrollingCourseIds.has(course._id)}
                      className={`px-4 py-2 text-sm rounded-lg ${
                        course.isEnrolled
                          ? "bg-gray-100 text-gray-500 cursor-not-allowed"
                          : enrollingCourseIds.has(course._id)
                          ? "bg-gray-300 text-gray-700 cursor-not-allowed"
                          : "bg-black text-white hover:bg-gray-800"
                      }`}
                    >
                      {enrollingCourseIds.has(course._id) ? "Enrolling..." : course.isEnrolled ? "Enrolled" : "Enroll"}
                    </button>
                  )}
                  {user?.role === "instructor" && (
